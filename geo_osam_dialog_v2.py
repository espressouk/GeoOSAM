...
def setup_pytorch_performance():
    ...
    try:
        ...
        # For high-core systems, use more aggressive threading
        if num_cores >= 16:
            # Use 75% of cores for computation, leave some for system
            optimal_threads = max(8, int(num_cores * 0.75))
        elif num_cores >= 8:
            # Use most cores but leave 1-2 for system
            optimal_threads = max(4, num_cores - 2)
        else:
            # For smaller systems, use almost all cores
            optimal_threads = max(1, num_cores - 1)

        print(f"🚀 Setting PyTorch to use {optimal_threads} threads")

        # Set PyTorch threading
        torch.set_num_threads(optimal_threads)
        torch.set_num_interop_threads(min(4, optimal_threads // 2))  # Limit interop threads
        ...
        print(f"✅ PyTorch configured: {actual_threads} compute threads, {torch.get_num_interop_threads()} interop threads")
...
class OptimizedSAM2Worker(QThread):
    ...
    def run(self):
        try:
            # Instead, just verify current settings
            if self.device == "cpu":
                current_threads = torch.get_num_threads()
                print(f"🧵 Worker using {current_threads} CPU threads")
            ...
            # Use torch.no_grad() and enable threading optimizations
            with torch.no_grad():
                # Enable autograd profiler for debugging (optional)
        ...
        try:
            ...
            def cpu_monitor():
                while True:
                    ...
                    print(f"💻 CPU Usage: {avg_cpu:.1f}% avg, {active_cores}/{len(cpu_percent)} cores active")
                    time.sleep(2)
            ...
            print("⚠️ psutil not available for CPU monitoring")
            return None
...
class GeoOSAMControlPanel(QtWidgets.QDockWidget):
    ...
    def _select_output_folder(self):
        ...
        if folder:
            ...
            self._update_status(f"📁 Output folder: {self.shapefile_save_dir}", "info")
    ...
    def _process_segmentation_result(self, mask, mask_transform, debug_info):
        ...
        try:
            ...
            # 2. Morphological OPEN to remove speckles
            open_kernel = np.ones((3, 3), np.uint8)  # Smaller kernel to keep details
            binary = cv2.morphologyEx(binary, cv2.MORPH_OPEN, open_kernel)

            # 3. Morphological CLOSE to fill small holes
            close_kernel = np.ones((7, 7), np.uint8)  # Larger kernel to close gaps
            binary = cv2.morphologyEx(binary, cv2.MORPH_CLOSE, close_kernel)

            # 4. Remove small objects (connected component filter)
            # --- This step gives you the most control! ---
            nlabels, labels, stats, _ = cv2.connectedComponentsWithStats(binary, connectivity=8)
            min_size = 20  # Adjust this (in pixels) for your mask resolution
            cleaned = np.zeros(binary.shape, dtype=np.uint8)
            for i in range(1, nlabels):  # 0 = background
                if stats[i, cv2.CC_STAT_AREA] >= min_size:
                    cleaned[labels == i] = 255
            binary = cleaned

        except Exception as e:
            self._update_status(f"Error thresholding/refining mask: {e}", "error")
            return
        ...
        try:
            for geom, _ in shapes(binary, mask=binary > 0, transform=mask_transform):
                ...
                if shp_geom.is_empty:
                    continue
                # if hasattr(shp_geom, 'area') and shp_geom.area < MIN_POLY_AREA:
                #     continue  # Skip tiny polygons
                print()
                self._update_status(f"Polygon area: {shp_geom.area}", "warning")
        ...
        self._update_status(f"✅ Added {len(feats)} [{self.current_class}] polygons!{undo_hint}", "info")
        self._update_stats()
