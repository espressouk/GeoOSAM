...
class OptimizedSAM2Worker(QThread):
    ...
    def _detect_object_candidates(self, image, bbox, class_name):
        ...
        # Choose detection method based on class
        if class_name in ['Vessels', 'Vehicle']:
            return self._detect_bright_objects(bbox_image, bbox, class_name)
        elif class_name in ['Buildings', 'Industrial']:
            return self._detect_rectangular_objects(bbox_image, bbox, class_name)
        elif class_name in ['Vegetation']:
            return self._detect_textured_objects(bbox_image, bbox, class_name)
        else:
            # Default: try bright object detection
            return self._detect_bright_objects(bbox_image, bbox, class_name)

    def _detect_bright_objects(self, bbox_image, bbox, class_name):
        """Detect bright objects (boats, vehicles) on darker backgrounds"""
        x1, y1, x2, y2 = bbox
        ...
        for i, contour in enumerate(contours):
            ...
            # Size filtering based on class
            min_area = self.min_object_size * 0.5  # Slightly smaller for detection
            max_area = 10000 if class_name == 'Vessels' else 20000
            ...
            if min_area <= area <= max_area:
                ...
                if M["m00"] != 0:
                    ...
                    # Additional shape validation for vessels/vehicles
                    x, y, w, h = cv2.boundingRect(contour)
                    aspect_ratio = max(w, h) / min(w, h)

                    if aspect_ratio <= 8.0:  # Reasonable aspect ratio
                        candidates.append((full_x, full_y))
                        print(f"  ✅ Candidate {len(candidates)}: ({full_x}, {full_y}) area={area:.0f}, aspect={aspect_ratio:.1f}")
                    else:
                        print(f"  ❌ Rejected: aspect ratio {aspect_ratio:.1f} too high")
                else:
                    print(f"  ❌ Rejected: could not calculate centroid")
            else:
                print(f"  ❌ Rejected: area {area:.0f} outside range [{min_area:.0f}, {max_area}]")

        print(f"  🎯 Found {len(candidates)} bright object candidates")
        return candidates

    def _detect_rectangular_objects(self, bbox_image, bbox, class_name):
        ...
        print(f"  🏢 Found {len(candidates)} rectangular object candidates")
        return candidates

    def _detect_textured_objects(self, bbox_image, bbox, class_name):
        """Detect textured objects (vegetation) using texture analysis"""
        x1, y1, x2, y2 = bbox
        ...
        # Apply local standard deviation filter to find textured areas
        kernel_size = 9
        ...
        texture_threshold = texture.mean() + texture.std()
        _, binary = cv2.threshold(texture.astype(np.uint8), int(texture_threshold), 255, cv2.THRESH_BINARY)
        ...
        print(f"  🌿 Found {len(candidates)} textured object candidates")
        return candidates
    ...
    def _validate_mask_for_class(self, mask, class_name, center_point):
        ...
        try:
            ...
            if class_name in ['Vessels', 'Vehicle']:
                return (
                    aspect_ratio <= 6.0 and      # Boats/vehicles shouldn't be too elongated
                    solidity >= 0.3 and          # Should be reasonably solid
                    contour_area <= 8000         # Not too large (reject water areas)
                )
            elif class_name in ['Buildings', 'Industrial']:
                return (
                    aspect_ratio <= 8.0 and
                    solidity >= 0.5              # Buildings should be more solid
                )
            elif class_name == 'Residential':
                # Enhanced validation for residential buildings
                min_threshold = max(self.min_object_size * 0.5, 25)
                return (
                    aspect_ratio <= 12.0 and     # Allow longer buildings
                    solidity >= 0.4 and          # More lenient solidity
                    contour_area >= min_threshold # Use relaxed size threshold
                )
            elif class_name in ['Vegetation']:
                return (
                    aspect_ratio <= 5.0 and
                    solidity >= 0.2              # Vegetation can be irregular
                )
            else:
                # Default validation
                return (
                    aspect_ratio <= 10.0 and
                    solidity >= 0.2
                )
    ...
    def _apply_class_specific_morphology(self, mask, class_name):
        ...
        if class_name in ['Vessels', 'Vehicle']:
            # For boats/vehicles: Aggressive separation with distance transform
            kernel_light = np.ones((2, 2), np.uint8)
            eroded = cv2.erode(mask, kernel_light, iterations=2)  # More erosion

            dist_transform = cv2.distanceTransform(eroded, cv2.DIST_L2, 5)
            _, sure_fg = cv2.threshold(dist_transform, 0.5 * dist_transform.max(), 255, 0)  # Higher threshold
            sure_fg = np.uint8(sure_fg)

            kernel_restore = np.ones((3, 3), np.uint8)
            final_mask = cv2.dilate(sure_fg, kernel_restore, iterations=3)  # More dilation
            final_mask = cv2.bitwise_and(final_mask, mask)

        elif class_name in ['Buildings', 'Industrial']:
            # For buildings: Moderate separation
            kernel_light = np.ones((2, 2), np.uint8)
            eroded = cv2.erode(mask, kernel_light, iterations=1)

            dist_transform = cv2.distanceTransform(eroded, cv2.DIST_L2, 5)
            _, sure_fg = cv2.threshold(dist_transform, 0.3 * dist_transform.max(), 255, 0)
            sure_fg = np.uint8(sure_fg)

            kernel_restore = np.ones((4, 4), np.uint8)
            final_mask = cv2.dilate(sure_fg, kernel_restore, iterations=2)
            final_mask = cv2.bitwise_and(final_mask, mask)

        elif class_name in ['Water', 'Agriculture']:
            # For large areas: Minimal separation
            kernel_light = np.ones((1, 1), np.uint8)
            final_mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, kernel_light)
